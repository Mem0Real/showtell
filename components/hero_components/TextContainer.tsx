import { AnimatePresence, motion, useTransform, useScroll } from "motion/react";

import { inter, playfair } from "@/lib/fonts";
import { MouseScrollIcon } from "@/components/hero_components/MouseScrollIcon";

export const TextContainer = ({ phase }: any) => {
  // Pulse animation variants
  const pulseVariants = {
    loading: {
      opacity: [0.6, 1, 0.6],
      transition: {
        duration: 1.5,
        repeat: Infinity,
        ease: "easeInOut",
      },
    },
  };

  // Text animation variants
  const textVariants = {
    initial: { opacity: 0 },
    loading: { opacity: 1 },
    split: (isLeft: Boolean) => ({
      x: isLeft ? "-19vw" : "15vw",
      opacity: 1,
      transition: { duration: 1.3, ease: [0.43, 0.13, 0.23, 0.96] },
    }),
    zoom: (isLeft: Boolean) => ({
      x: 0,
      opacity: 1,
      transition: { duration: 1.0, ease: "easeIn" },
    }),
    complete: {
      x: 0,
      opacity: 0,
      transition: {
        duration: 0.8,
        delay: 0.3,
        ease: "easeOut",
      },
    },
  };

  return (
    <>
      {/* Split Text */}
      <motion.div
        className={`relative z-10 w-full h-full flex justify-center items-center gap-6 ${playfair.className}`}
        animate={phase === "loading" ? "loading" : undefined}
        variants={pulseVariants as any}
      >
        <motion.h1
          custom={true}
          variants={textVariants as any}
          initial="initial"
          animate={phase}
          className={`${playfair.className} text-3xl md:text-5xl lg:text-9xl font-bold text-stone-800`}
        >
          SHOW
        </motion.h1>

        <motion.h1
          custom={false}
          variants={textVariants as any}
          initial="initial"
          animate={phase}
          className={`${playfair.className} text-3xl md:text-5xl lg:text-9xl font-bold text-stone-800`}
        >
          TELL
        </motion.h1>
      </motion.div>

      {/* Overlay Content (appears after zoom) */}
      {phase === "complete" && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.8 }}
          className="absolute bottom-0 left-1/2 -translate-x-1/2 z-30 pointer-events-none flex flex-col justify-center items-center w-full md:w-fit"
        >
          <div className="h-full w-full xl:px-36 lg:px-12 md:px-4 py-4 bg-black/30 rounded-sm bg-clip-padding border border-b-0 border-white/10 flex flex-col items-center justify-center gap-2">
            <motion.h2
              initial={{ y: 30, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ duration: 0.6, delay: 0.3 }}
              className={`text-white text-2xl md:text-4xl font-extralight pb-4 capitalize ${playfair.className}`}
            >
              Welcome to showtell
            </motion.h2>
            <motion.p
              initial={{ y: 30, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ duration: 0.6, delay: 0.5 }}
              className="text-white/80 md:text-lg capitalize"
            >
              Experience your vacation from home
            </motion.p>
            <motion.button className="transition-all duration-500">
              <MouseScrollIcon />
            </motion.button>
          </div>
        </motion.div>
      )}

      {/* Loading indicator */}
      {phase === "loading" && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="absolute bottom-8 left-1/2 -translate-x-1/2 z-30 text-black/60 text-sm"
        >
          Loading...
        </motion.div>
      )}
    </>
  );
};
